...
def main(args):
    ...
    """if args.eval:
        ...
        print(f"Checkpoint model keys: {list(checkpoint_model.keys())}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == 300 :
            print("time to do the test !!!!")
            args.test_path = "/home/maloulou/UCMerced_LandUse/test.csv"
            dataset_val = build_fmow_dataset(is_train=False, args=args)
            if args.dist_eval:
                if len(dataset_val) % num_tasks != 0:
                    print('Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
        ...
        # Save model only if it's the epoch with max_accuracy
        if args.output_dir and epoch == max_accuracy_epoch :
        # Remove old checkpoint files if needed
           for file in os.listdir(args.output_dir):
                if file.endswith(".pth") and file.startswith("checkpoint"):
