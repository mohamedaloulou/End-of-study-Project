...
def get_args_parser():
    ...
    parser.add_argument('--step_size', type=int, nargs='+', default=[33, 46], help='Step size for learning rate decay')
    ...
    parser.add_argument('--output_dir', default='/data/maloulou/scratch_resisc', help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='/data/maloulou/scratch_resisc', help='path where to tensorboard log')
    parser.add_argument('--device', default='cuda', help='device to use for training / testing')
    parser.add_argument('--seed', default=0, type=int)
    parser.add_argument('--resume', default=None, help='resume from checkpoint')
    parser.add_argument('--save_every', type=int, default=50, help='How frequently (in epochs) to save ckpt')
...
def main(args,subset_train_indices,subset_test_indices,log_file):
    ...
    dataset_train = build_fmow_dataset(is_train=True, args=args,subset_indices=subset_train_indices)
    dataset_val = build_fmow_dataset(is_train=False, args=args,subset_indices=subset_test_indices)
    ...
    if args.eval:
        test_stats,accuracy_perclass = evaluate(data_loader_val, model, device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     ...
                     **{f'class_{cls}_acc': acc for cls, acc in accuracy_perclass.items()},
                     'epoch': epoch,
        ...
        if args.output_dir and misc.is_main_process():
            if log_writer is not None:
                log_writer.flush()
            with open(os.path.join(args.output_dir, log_file), mode="a", encoding="utf-8") as f:
...
if __name__ == '__main__':
    ...
    train_file='/home/maloulou/ucmerced_train_indices.txt'
    test_file = '/home/maloulou/ucmerced_validation_indices.txt'
    ...
    """num_train_samples = int(27000 * 0.05)
    subset_train_indices = np.random.choice(27000, num_train_samples, replace=False)
    num_test_samples = int(4500 * 0.05)
    subset_test_indices = np.random.choice(4500, num_test_samples, replace=False)"""
    # Define hyperparameter grid
    learning_rates = [0.00005,0.00006,0.00007,0.00008,0.00009,0.0001,0.0002]
    batch_sizes = [4]
    patch_sizes = [8]
    # Create combinations of hyperparameters
    param_grid = list(itertools.product(learning_rates, batch_sizes, patch_sizes))
        
    for learning_rate, batch_size, patch_size in param_grid:
        args.lr = learning_rate
        args.batch_size = batch_size
        args.patch_size = patch_size

        # Generate a unique log file name for each combination
        log_file = f"log_lr{learning_rate}_bs{batch_size}_ps{patch_size}.txt"
        print(f"Running with learning_rate={learning_rate}, batch_size={batch_size}, patch_size={patch_size}")

        # Run the main function and get max_accuracy
        max_accuracy = main(args, train_indices, test_indices, log_file)
    print("DONE !")
